...
//    }
    ...
    public boolean enregistrerDossier(Dossier dossier) {
        ...
        try {
            ...
            if(isSaved > 0){
                ...
                //todo : re overlrid the total method update
                this.updateTotoalRem(dossier.getNumDossier());
                return true;
            }
    ...
    private void updateTotoalRem(String numDossier) {
        //todo : i need cluaclue
        try{
            String Query = "UPDATE dossiers SET totalRemboursement = ? WHERE numDossier = ?";
            float totalToReimburse = this.CalculateTotalAnalyse(numDossier) + this.CalculateTotalMedicament(numDossier) + this.CalculateTotalRadio(numDossier) + this.CalculateTotalVisiste(numDossier);
            PreparedStatement preparedStatement = connection.prepareStatement(Query);
            preparedStatement.setFloat(1,totalToReimburse);
            preparedStatement.setString(2,numDossier);
            preparedStatement.executeUpdate();
        }catch (SQLException e){
            e.printStackTrace();
        }
        //this.totalRemoursement(numDossier);
    ...
    public  int insertFicher(Dossier dossier){
        try{
            ...
            insertFicherStatement.setString(5,dossier.getNumDossier());
           return insertFicherStatement.executeUpdate();
